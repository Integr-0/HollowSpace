...
public class EnemyPathfind : MonoBehaviour {
    ...
    private List<Vector2> FindPath(Vector2 start, Vector2 goal) {
        ...
        var openSet = new List<Node> { new Node(start, null, 0, Vector2.Distance(start, goal)) };
        ...
        while (openSet.Count > 0) {
            ...
            foreach (var neighbor in GetNeighbors(currentNode.Position)) {
                if (closedSet.Contains(neighbor) || IsIlluminated(neighbor)) continue;
                ...
                if (existingNode == null) {
                    openSet.Add(new Node(neighbor, currentNode, tentativeG, Vector2.Distance(neighbor, goal)));
                }
